...
#podman run --replace -d --privileged --name $CONTAINER_NAME -p $LDAP_PORT:389 -p $LDAPS_PORT:636 -p $ADMIN_PORT:9090 localhost/$IMAGE_NAME /sbin/init
podman run --pull=never --replace -d --privileged --name $CONTAINER_NAME -p $LDAP_PORT:389 -p $LDAPS_PORT:636 -p $ADMIN_PORT:9090 $IMAGE_NAME /sbin/init
sleep 3
...
ldapadd -V -H ldap://localhost:10389 -D "cn=Directory Manager" -w password -f users-example.ldif
ldapadd -V -H ldap://localhost:10389 -D "cn=Directory Manager" -w password -f groups-example.ldif
